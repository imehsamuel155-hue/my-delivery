...
window.renderShipDetail = function renderShipDetail(shipment) {
    ...
    <div class="field">
      ...
      <input id="f_code" class="mono" value="${esc(s.code)}" data-original-code="${esc(s.code)}"  placeholder="e.g. DHL XXX">
    </div>
    <div class="field">
...
window.saveShipment = async function saveShipment(isNew) {
    ...
    try {
        ...
        if (isNew) {
            ...
            const urlCode = (document.getElementById('f_code') && document.getElementById('f_code').getAttribute('data-original-code')) || code;
            payload.code = code;
            updated = await apiRequest('/shipments/' + encodeURIComponent(urlCode), { method: 'PUT', body: JSON.stringify(payload) });
        }
